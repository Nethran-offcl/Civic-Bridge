    ...
    <form onSubmit={submit} className="space-y-2 border-t border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-slate-950">
      {warning ? (
        <div className="flex items-center gap-2 rounded-md bg-rose-50 px-3 py-2 text-sm font-medium text-rose-700 dark:bg-rose-500/10 dark:text-rose-200">
          ...
          {t("Do not share Aadhaar, OTP, passwords, or bank details here.")}
        </div>
      ) : null}
      <div className="flex gap-2">
        <Input
          ...
          placeholder={t("Ask about documents, eligibility, or application steps")}
          disabled={disabled}
        ...
        <Button
          ...
          title={supportsSpeech ? t("Speak question") : t("Voice input is available in supported Chrome browsers")}
        >
          {isListening ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}
        </Button>
        <Button type="submit" size="icon" disabled={disabled || !message.trim() || warning} title={t("Send message")}>
